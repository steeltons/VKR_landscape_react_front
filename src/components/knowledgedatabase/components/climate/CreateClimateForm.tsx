...
const CreateReliefForm = ({ onCancel, id, insertName, insertDescription, isUpdate, canUserEdit  }: Props) => {
  ...
  const { enqueueSnackbar } = useSnackbar();
   ...
   const handleUploadObject = async (data: UpdateClimatRqDto) => {
      try {
        const response = await updateClimat(data);
  ...
  const handleCreateObject = async (data: CreateClimatRqDto) => {
    try {
      const response = await insertClimat(data);
      enqueueSnackbar(response.message, { variant: 'success' })
    } catch (error: any) {
        enqueueSnackbar('Что-то пошло не так', { variant: 'error' })
    }
  ...
  const handleSubmit = () => {
    const upload = async () => {
      ...
      if (isUpdate) {
        const updateRqDto : UpdateClimatRqDto = {
          climat_id: id,
          climat_name: name,
          climat_description: description,
          climat_picture_id: imageId
        }
        handleUploadObject(updateRqDto);
      } else {
        const plantRqDto : CreateClimatRqDto = {
          climat_name: name,
          climat_description: description,
          climat_picture_id: imageId
      }
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Button variant="outlined" onClick={onCancel} sx={{ mb: 2 }}>
        ← Вернуться к климатам
      </Button>
      <Typography variant="h5" gutterBottom>
        ➕ {(formId === undefined) ? 'Добавление нового климата' : `Редактирование климата ${name}`}
      </Typography>
