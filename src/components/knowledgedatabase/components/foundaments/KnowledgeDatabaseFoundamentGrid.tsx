...
import CreateReliefForm from "./CreateFoundamentForm";
import { deleteFoundation, FoundationRsDto, getAllFoundations } from "../../../../services/foundaments";
...
function convertToKnowledfeDatabaseObjectUi(response : FoundationRsDto) : KnowledgeDatabaseObjectUi {
    return {
        id: response.foundation_id,
        name: response.foundation_name,
        description: response.foundation_description,
        depth: response.foundation_depth_roof_root_in_meters,
        base64Image: response.foundation_picture_base64
    }
}
...
const KnowledgeDatabaseFoundationGrid = () => {
    ...
    useEffect(() => {
        const formData = async () => {
            try {
                const response = await getAllFoundations(true);
    ...
    if (isCreating) {
        return <CreateReliefForm 
            ...
            insertDepth={ selectedObject?.depth || 0 }
...
export default KnowledgeDatabaseFoundationGrid;